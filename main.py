...
# --- 4. ПАНЕЛЬ АРХИТЕКТОРА (В SIDEBAR) ---
...
if input_pass == MASTER_PASSWORD:
    ...
    if st.sidebar.button("🚨 ЗАПУСТИТЬ SOS В ГРУППУ"):
        text = "🚨 ВНИМАНИЕ! АРХИТЕКТОР ОБЪЯВИЛ ОБЩИЙ СБОР! 🚨\nВСЕМ УЧАСТНИКАМ AD1SKA SQUAD СРОЧНО ВЫЙТИ НА СВЯЗЬ!"
        
        # ЖЕЛЕЗОБЕТОННАЯ ССЫЛКА
        url = f"https://api.telegram.org{BOT_TOKEN}/sendMessage"
        
        try:
            res = requests.post(url, data={"chat_id": GROUP_ID, "text": text}, timeout=10)
            if res.status_code == 200:
                st.sidebar.snow()
                st.sidebar.success("СИГНАЛ УШЕЛ В @ad1skasquad!")
            else:
                st.sidebar.error(f"Ошибка: {res.json().get('description', 'Неизвестно')}")
